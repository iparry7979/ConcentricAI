class constants
{
	firstNameMale = ["Tim", "Luke", "Leigh", "Justin", "Aaron", "Michael", "David", "Matthew", "Jacob", "Ian", "Nathanael", "Antoine", 
					"Pascal", "Phillip","Furqan", "Lachlan", "Adam", "Ben", "Lisa", "Shane", "Darryn", "Keith", "James", "Jack", 
					"Francis", "Tristan", "Cody", "Eran", "Nathan", "Rupert", "Daniel", "Vince", "Sean", "Angus", "Anthony", 
					"John", "Rahul", "Blake", "Anto", "Steve", "Pat"];
					
	firstNameFemale = ["Arwen", "Ree", "Jane", "Sarah", "Ashley", "Loz", "Jemma",  "Aurora", "Kathryn", "Geraldine", "Laura",
					 "Rebecca",  "Liese", "Ambrosia", "Jillian", "Lisa", "Lilla", "Chantel", "Jessica", "Dani", "Rachel", "Danielle", 
					 "Linden", "Katrina", "Kirsten", "Emma", "Bridie", "Wendy", "Susanna", "Isobel", "Ndali", "Moniker", "Serena", 
					 "Trish", "Emily", "Aimee"];
					 
	lastName = ["Garnsey", "McKelvey", "Sanderson", "Tyson-Parry", "Best", "Parry", "Pearce", "Emmerton", "Lorch", 
				"Cayeaux", "McPherson", "Karban", "Myors", "Foley", "Sunrise", "Samuel", "Porto", "Mackintosh", 
				"Chapman", "Busch", "Tiger", "Rennie", "Scholz", "Arshad", "Coote", "Peanna", "MacDonald", "Lilburne", 
				"Stewart", "Paynter", "Tair", "Wilson", "St Quintin", "Rose", "Moran", "Farrant", "McLoughlin", "Govett", "Burch", 
				"Leigh", "Fitzpatrick", "Johnston", "Bromwich", "Peat", "Winter", "Roberts", "Danger", "Walker", "Tongue", "Amobi", 
				"Gronk", "Hammond", "Davies", "David", "Mulder", "Butcher", "Mirovar", "Thyssen", "Hulbert", "Harris", "Tyson", 
				"Gollan", "Loomes", "Duckett", "Mazzaferri"];
}

